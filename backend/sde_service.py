...
async def import_data():
    ...
    async with AsyncSessionLocal() as session:
        ...
        cursor.execute(
            "SELECT marketGroupID, parentGroupID, marketGroupName, description, hasTypes FROM invMarketGroups"
        )
        ...
        cursor.execute(
            "SELECT typeID, typeName, groupID, volume, marketGroupID FROM invTypes WHERE marketGroupID IS NOT NULL"
        )
        types = cursor.fetchall()
        for i, t in enumerate(types):
            item = SdeType(
                type_id=t[0], name=t[1], group_id=t[2], volume=t[3], market_group_id=t[4]
            )
        ...
        cursor.execute(
            "SELECT solarSystemID, regionID, solarSystemName, security FROM mapSolarSystems"
        )
        systems = cursor.fetchall()
        for i, s in enumerate(systems):
            system = SdeSolarSystem(system_id=s[0], region_id=s[1], name=s[2], security=s[3])
        ...
        for i, j in enumerate(jumps):
            jump = SdeSolarSystemJump(from_solar_system_id=j[0], to_solar_system_id=j[1])
        ...
        for i, st in enumerate(stations):
            station = SdeStation(station_id=st[0], solar_system_id=st[1], name=st[2])
