...
from sqlalchemy import func, or_, delete, text
from database import get_db
from models import MarketOrder as MarketOrderModel, SdeType, SdeRegion, SdeStation, MarketHistory, SdeMarketGroup, SdeSolarSystem
from esi_client import esi_app, esi_client
from worker import fetch_market_orders
...
async def search_types(q: str, db: AsyncSession = Depends(get_db)):
    ...
    return await paginate(db, stmt, transformer=lambda items: [
...
async def search_locations(q: str, db: AsyncSession = Depends(get_db)):
    ...
    for s in systems:
        results.append({"id": s.system_id, "name": s.name, "type": "system", "parent_id": s.region_id})
        
    for st in stations:
        results.append({"id": st.station_id, "name": st.name, "type": "station", "parent_id": st.solar_system_id})
...
async def get_market_groups_tree(db: AsyncSession = Depends(get_db)):
    ...
    """
    stmt = select(SdeMarketGroup.market_group_id, SdeMarketGroup.parent_group_id, SdeMarketGroup.name, SdeMarketGroup.has_types)
    result = await db.execute(stmt)
    groups = result.all()
    return [
        {
...
async def get_group_types(group_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return await paginate(db, stmt, transformer=lambda items: [
        {"id": item.type_id, "name": item.name} 
        for item in items
    ])
...
async def get_market_history(
    ...
    stmt = select(MarketHistory).where(
