...
import { Paper, Title, TextInput, Stack, Text, Button, Loader, NavLink, ScrollArea, Box, Group, CloseButton, Badge } from '@mantine/core';
import axios from 'axios';
import { useDebouncedValue } from '@mantine/hooks';
import EveIcon from './EveIcon';

const FolderIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.7 }}>
...
function MarketGroupTree({ groupChildrenMap, parentId, onSelectType, onSelectGroup, selectedTypeId, selectedGroupId, expandedGroups, toggleGroup, filteredGroupIds, injectedItems, isFiltering }) {
...
function MarketGroupNode({ group, groupChildrenMap, onSelectType, onSelectGroup, selectedTypeId, selectedGroupId, expandedGroups, toggleGroup, filteredGroupIds, injectedItems, isFiltering }) {
  const [types, setTypes] = useState([]);
  const [loadingTypes, setLoadingTypes] = useState(false);
  const isExpanded = expandedGroups.has(group.id);

  // Determine which items to show
  const displayItems = (isFiltering && injectedItems.has(group.id)) 
    ? injectedItems.get(group.id) 
    : types;  // When not filtering, always show types (which are loaded on demand)
  ...
  useEffect(() => {
    ...
    if (isExpanded && group.has_types && types.length === 0 && (!isFiltering || !injectedItems.has(group.id))) {
      setLoadingTypes(true);
      axios.get(`http://localhost:8000/api/market/groups/${group.id}/types?size=250`)
        .then(res => {
        ...
        <Box pl="md">
          ...
          {sortedItems.map(type => (
            <NavLink
              key={type.id}
              label={type.name}
              leftSection={<EveIcon id={type.id} name={type.name} size={24} useAvatar radius="xs" />}
...
function ItemFilterWidget({ onSelect, selectedItem, onSelectGroup, selectedGroup, searchValue, onSearchChange, resetTrigger }) {
  ...
  useEffect(() => {
    ...
    const fetchAndCache = () => {
      axios.get('http://localhost:8000/api/market/groups/tree')
        .then(res => {
          setGroups(res.data);
  ...
  useEffect(() => {
    if (isFiltering) {
      ...
      axios.get(`http://localhost:8000/api/market/types/search?q=${debouncedSearch}&size=250`)
        .then(res => {
  ...
  return (
    <Paper shadow="xs" p="md" withBorder display="flex" style={{ flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <Title order={4} mb="sm" size="h5">Item Browser ({groups.length})</Title>
      
      {/* Selected Filters Section */}
      {(selectedItem || selectedGroup) && (
        <Stack mb="sm" gap="xs">
          <Text size="xs" fw={600} c="dimmed">Selected Filter:</Text>
          {selectedItem && (
            <Badge
              size="lg"
              variant="light"
      ...
      <Text size="xs" fw={500} mb="xs">Market Groups</Text>
      <ScrollArea style={{ flex: 1, minHeight: 0 }} type="auto" scrollbarSize={6}>
        {groupsLoading ? (
          ...
          <Text size="sm" c="dimmed" ta="center" mt="md">No market groups found</Text>
        )}
      </ScrollArea>
    </Paper>
  );
