...
const EveIcon = ({ id, name, type = 'type', variant = 'icon', size = 32, useAvatar = false, maxRetries = 3, ...props }) => {
    ...
    const url = `${getEveIconUrl(id, name, type, variant)}?size=${targetSize}${retryCount > 0 ? `&retry=${retryCount}&t=${Date.now()}` : ''}`;
    ...
    const colors = ['blue', 'red', 'green', 'yellow', 'orange', 'purple', 'teal', 'cyan', 'pink', 'grape'];
