...
import { Table, Title, Loader, Badge, Paper, TextInput, Select, Group, Stack, Image, Text, HoverCard, Button, Collapse, Center, ScrollArea, Box } from '@mantine/core';
...
const ItemHoverCard = React.memo(function ItemHoverCard({ typeId, typeName, children }) {
  ...
  const fetchDetails = useCallback(() => {
    if (details) return;
    setLoading(true)
    axios.get(`http://localhost:8000/api/market/types/${typeId}/details`)
      .then(res => {
        setDetails(res.data)
        setLoading(false)
      })
  ...
  return (
    <HoverCard width={320} shadow="md" onOpen={fetchDetails}>
      ...
      <HoverCard.Dropdown>
        <Text size="sm" fw={500} mb="xs">{typeName}</Text>
        {loading ? (
          <Loader size="sm" />
        ) : details ? (
          <Stack gap="xs">
            ...
            <Group gap="xs">
              <Badge size="xs" variant="outline">Volume: {details.volume} m3</Badge>
              {details.mass && <Badge size="xs" variant="outline">Mass: {details.mass} kg</Badge>}
            </Group>
          </Stack>
        ) : (
...
const MarketHistoryChart = React.memo(function MarketHistoryChart({ regionId, typeId, colorScheme = 'light' }) {
  const [history, setHistory] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setLoading(true)
    axios.get(`http://localhost:8000/api/market/history`, { params: { region_id: regionId, type_id: typeId } })
      .then(res => {
        setHistory(res.data)
        setLoading(false)
      })
  ...
  if (!history.length) return <Text size="sm" c="dimmed">No history available</Text>
  ...
  if (colorScheme === 'dark') {
    Highcharts.setOptions({
      ...
      xAxis: { labels: { style: { color: '#b0b0b0' } }, title: { style: { color: '#e0e0e0' } }, lineColor: '#333333', tickColor: '#333333' },
      yAxis: { labels: { style: { color: '#b0b0b0' } }, title: { style: { color: '#e0e0e0' } }, lineColor: '#333333', tickColor: '#333333', gridLineColor: '#2a2a2a' },
    ...
    Highcharts.setOptions({
      ...
      xAxis: { labels: { style: { color: '#666666' } }, title: { style: { color: '#000000' } }, lineColor: '#e0e0e0', tickColor: '#e0e0e0' },
      yAxis: { labels: { style: { color: '#666666' } }, title: { style: { color: '#000000' } }, lineColor: '#e0e0e0', tickColor: '#e0e0e0', gridLineColor: '#f0f0f0' },
  ...
  const options = {
    ...
    series: (function() {
      ...
      // Build series data
      const priceRangeSeries = history.map(h => [new Date(h.date).getTime(), h.lowest, h.highest])
      ...
      const dailyRangeSeries = history.map(h => [new Date(h.date).getTime(), h.highest - h.lowest])
...
function MarketDashboard({ selectedItem, selectedGroup, selectedLocation, orderType, colorScheme = 'light', onItemSearchIconClick, onLocationSearchIconClick }) {
  ...
  const fetchOrders = useCallback((pageNum = 1, append = false) => {
    ...
    axios.get('http://localhost:8000/api/market/orders', { params })
      .then(res => {
        console.log('Received orders:', { pageNum, itemsCount: res.data.items.length, totalPages: res.data.pages })
        ...
        const hasMorePages = pageNum < res.data.pages && (append ? (orders.length < MAX_ROWS) : (res.data.items.length > 0))
  ...
  const handleStationClick = useCallback((order) => {
    ...
    if (!q) return
    axios.get('http://localhost:8000/api/market/locations/search', { params: { q } })
      .then(res => {
  ...
  return (
    <Paper shadow="xs" p="md" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Title order={2} mb="md">Market Dashboard</Title>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        {loading && page === 1 ? (
          ...
          <Center mt="xl">
            <Text size="sm" c="dimmed">No orders found</Text>
          </Center>
        ) : (
          <>
            <Box ref={parentRef} style={{ flex: 1, overflow: 'auto' }}>
              {/* Header rendered as CSS grid to match body column layout */}
              <div style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: 'var(--mantine-color-body)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '20% 10% 8% 12% 10% 20% 12% 8%', gap: 0, padding: '8px 12px', fontWeight: 600 }}>
                  <div>Item</div>
                  <div>Region</div>
                  <div>Type</div>
                  <div>Price</div>
              ...
              <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: 'relative', width: '100%' }}>
                {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                  ...
                  const rowBg = virtualRow.index % 2 === 0 ? 'transparent' : 'rgba(255, 255, 255, 0.04)'
                  ...
                  return (
                    ...
                    >
                      <div role="row" style={{ display: 'grid', gridTemplateColumns: '20% 10% 8% 12% 10% 20% 12% 8%', gap: 0, padding: '8px 12px', alignItems: 'center', background: rowBg, cursor: 'pointer' }} onClick={() => toggleRow(order.order_id)}>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', minWidth: 0 }}>
                          <ItemHoverCard typeId={order.type_id} typeName={order.type_name}>
                            <Group gap="xs" style={{ alignItems: 'flex-start', flex: 1, minWidth: 0 }}>
                              <EveIcon id={order.type_id} name={order.type_name} size={32} />
                              <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', minWidth: 0 }}>
                                <Text
                                  size="sm"
                                  style={{ wordBreak: 'break-word', minWidth: 0, cursor: 'pointer', textDecoration: 'underline dotted', textDecorationColor: 'rgba(128,128,128,0.6)' }}
                                  tabIndex={0}
                                  role="button"
                                  onClick={(e) => { e.stopPropagation(); if (onItemSearchIconClick) onItemSearchIconClick({ id: order.type_id, name: order.type_name }) }}
                                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); if (onItemSearchIconClick) onItemSearchIconClick({ id: order.type_id, name: order.type_name }) } }}
                                >
                                  {order.type_name}
                                </Text>
                              </div>
                              ...
                              <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', minWidth: 0 }}>
                                <Text
                                  size="sm"
                                  style={{ minWidth: 0, cursor: 'pointer', textDecoration: 'underline dotted', textDecorationColor: 'rgba(128,128,128,0.6)' }}
                                  tabIndex={0}
                                  role="button"
                                  onClick={(e) => { e.stopPropagation(); if (onLocationSearchIconClick) onLocationSearchIconClick({ type: 'region', id: order.region_id, name: order.region_name }) }}
                                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); if (onLocationSearchIconClick) onLocationSearchIconClick({ type: 'region', id: order.region_id, name: order.region_name }) } }}
                                >
                                  {order.region_name}
                                </Text>
                              </div>
                        ...
                        <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{order.price.toLocaleString()} ISK</div>
                        <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{order.volume_remain.toLocaleString()}</div>
                              ...
                              <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', minWidth: 0 }}>
                                <Text
                                  ...
                                  tabIndex={0}
                                  role="button"
                                  onClick={(e) => { e.stopPropagation(); handleStationClick(order) }}
                                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); handleStationClick(order) } }}
                                >
                                  {order.station_name}
                                </Text>
                              </div>

                        <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{formatDate(order.issued)}</div>
                        <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{formatTimeRemaining(order.issued, order.duration)}</div>
                      </div>
                        ...
                        <div style={{ background: rowBg }}>
                          <div style={{ gridColumn: '1 / -1', padding: 8 }}>
                            <Paper p={8} withBorder style={{ margin: 0 }}>
                              <MarketHistoryChart regionId={order.region_id} typeId={order.type_id} colorScheme={colorScheme} />
                            </Paper>
                          </div>
                        </div>
                      )}
              ...
              {loading && page > 1 && (
                <Center mt="md" mb="md">
                  <Stack align="center" gap="sm">
                    <Loader size="md" />
                    <Text size="sm" c="dimmed">Loading more orders...</Text>
                  </Stack>
                </Center>
              )}
              ...
              {!hasMore && (
                <Center mt="md" mb="md">
                  <Paper p="md" withBorder radius="md" bg="gray.0" style={{ borderStyle: 'dashed' }}>
